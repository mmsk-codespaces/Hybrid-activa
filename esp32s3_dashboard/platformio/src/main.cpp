/**
 * main.cpp  (PlatformIO)
 * ESP32 — ReynLab Hybrid Activa WiFi Dashboard
 * ReynLab Technologies India Pvt. Ltd.
 *
 * Receives JSON from Arduino Mega 2560 via Serial2 (UART2 — GPIO16 RX, GPIO17 TX)
...
// ================================================================
// GPIO16/GPIO17 are used by onboard PSRAM (N16R2) — do NOT use them
// Using Serial2 remapped to safe pins:
//   GPIO13 (D7) ← Mega TX3 Pin14  via 1kΩ+2.2kΩ voltage divider (5V→3.3V)
//   GPIO14 (D6) → Mega RX3 Pin15  direct (3.3V is valid HIGH for Mega)
#define MEGA_RX_PIN   13
#define MEGA_TX_PIN   14
...
struct {
  ...
  unsigned long icToEvHoldMs = 120000UL;
  float         icLowRpm  = 2500.0f;
...
<div class="grid3">
  <div class="card">
    ...
    <div class="tune-row">
      <input type="number" id="evHoldInput" min="1" max="120" step="1" value="5">
  ...
  <div class="card">
    ...
    <div class="val tune" id="icHoldDisp">120</div>
    <div class="unit">sec (on Mega)</div>
    <div class="tune-row">
      <input type="number" id="icHoldInput" min="1" max="120" step="1" value="120">
  ...
  <div class="card">
    ...
    <div class="val tune" id="icThrDisp">2500</div>
    <div class="unit">RPM (on Mega)</div>
    <div class="tune-row">
      <input type="number" id="icThrInput" min="1000" max="6000" step="100" value="2500">
...
async function setEvHold(){
  ...
  if(isNaN(v)||v<1||v>120){fb.style.color='#FF4455';fb.textContent='Enter 1-120';return;}
...
async function setIcThr(){
  ...
  if(isNaN(v)||v<1000||v>6000){fb.style.color='#FF4455';fb.textContent='Enter 1000-6000';return;}
...
async function poll(){
  try{
    ...
    // Determine IC active state FIRST — drives all display decisions
    const icActive=(d.state==='IC_ONLY'||d.state==='TRANS_TO_IC');
    // Hub Motor RPM — hide during IC (engine drives wheel, not EV controller)
    set('hubRPM', icActive ? '--' : Math.round(d.hubRPM));
    // IC Engine RPM — hide during EV (engine off)
    ...
    // Tuning values (reflect live Mega values)
...
void handleSetEvHold() {
  ...
  if (s < 1 || s > 120) { server.send(400, "text/plain", "out of range"); return; }
...
void handleSetIcThr() {
  if (!server.hasArg("val")) { server.send(400, "text/plain", "missing val"); return; }
  float v = server.arg("val").toFloat();
  if (v < 1000.0f || v > 6000.0f) { server.send(400, "text/plain", "out of range"); return; }
  live.icLowRpm = v;
...
void setup() {
  Serial.begin(115200);
  Serial2.begin(MEGA_BAUD, SERIAL_8N1, MEGA_RX_PIN, MEGA_TX_PIN);  // GPIO13=RX, GPIO14=TX
